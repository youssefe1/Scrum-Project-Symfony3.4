...
class SprintController extends Controller
{
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $sql = "select * from backlog_sprint INNER join projets on backlog_sprint.id_projet=projets.id_projet INNER JOIN equipe on equipe.ide_projet=projets.ide_equipe INNER JOIN fos_user on fos_user.id=equipe.ide_scrum_master WHERE fos_user.username=:dom";
        $params[':dom'] = $nom;
        $em = $this->getDoctrine()->getManager();
        $stmt = $em->getConnection()->prepare($sql);
        $stmt->execute($params);
